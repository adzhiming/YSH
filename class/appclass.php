...
class appclass
{
	...
	function __construct(){ 	  
	  	...
	  	$this->otherlink = '';
		$this->noticetype = 1;
		$this->xmbao = Mysite::$app->config['xmbao'];//小米包名
		$this->xmtitle =  Mysite::$app->config['xmtitle'];//小米推送标题
		$this->miuiKey =  Mysite::$app->config['miuiKey'];;//小米的key值
   ...
   {   
    ...
    function sendNewmsg($message,$othermsg=''){ 
	   if(!empty($this->gtuserids)){
			$this->otherlink = '&userID='.$this->gtuserids.'&channelid=&message='.$message.'&othermsg='.$othermsg.'&messagetype=1'; 
			$code = $this->bklink();
			if(!empty($othermsg)){
				$this->xmmsg = $othermsg;
