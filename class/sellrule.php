...
class sellrule
{
    ...
    private $rulecontrol = array('1'=>'赠送','2'=>'减费用','3'=>'折扣','4'=>'免配送费');
     ...
     * $shoptype 店铺类型
     * $userid 用户id
     * $platform 支持平台：1pc端，2微信端，3触屏端，4客户端（安卓苹果）
     * $paytepe  1货到付款  2在线支付
     */
  ...
  public function get_rulelist(){
  	$list = array();
        ...
        if($smardb->setdb($this->mysql)->SetShopId($this->shopid)->OneShop()) {
            $carinfo = $smardb->getdata();
            foreach($carinfo['goodslist'] as $v){
                if($v['is_cx']==1){
                    return $list;
                }
            }
        ...
        $list = $this->mysql->getarr("select * from ".$this->tablepre."rule where FIND_IN_SET('$this->shopid',shopid) and status = 1 and  starttime < ".$this->maketime." and endtime > ".$this->maketime." ");
        return $list;
    }
    //返回规则数据
    ...
    function maxdata($data,$gzdata)
    {
        ...
        if(isset($data['zhekou']))
        {
	        ...
	        $down1 = $makedata['surecost']*(1-$findcost*0.01);
        ...
        foreach($list as $key=>$value)
        {
            if($value['type'] == 1)
            {
                ...
                if(in_array($this->platform,$supportplatform) || $this->platform == 0){//判断在那个端购买
                    if((!in_array('1',$supporttype)) || (in_array('1',$supporttype) && $this->firstorder == 0)){//判断是否是首次下单
                        if((!in_array('2',$supporttype)) || (in_array('2',$supporttype) && $this->paytype == 1)) {//判断是否是在线支付
                            //表示购物车总价  //默认都是 购物车总价
                            if ($value['limitcontent'] <= $this->cartcost) {//商品总价 > 当前购物车
                                ...
                                } elseif ($value['limittype'] == 3) {//指定时间间隔
