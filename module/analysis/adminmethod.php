...
class method   extends adminbaseclass
{
    function area(){
        ...
        foreach($arealist as $key=>$value){// FIND_IN_SET('".$firstareain."',`areaids`)
            $where = empty($wherearray[$tempselecttype])?" where admin_id=".$value['adcode']." ":$wherearray[$tempselecttype]." and where admin_id=".$value['adcode']." ";
    ...
    function user(){
        ...
        $tempdata =   $this->mysql->getarr("select count(id) as shuliang ,DATE_FORMAT(FROM_UNIXTIME(`addtime`),'%k') as month from ".Mysite::$app->config['tablepre']."order  ".$wherearray[$tempselecttype]." group by month    order by month desc  limit 0,25");
        $list = array();
    ...
    function orderyjin()
    {
        ...
        if(is_array($shoplist))
        {
            foreach($shoplist as $key=>$value)
            {
                ...
                /*
				8.6新增字段
				shopdowncost：优惠金额中平台承担的部分
				（cxcost- shopdowncost）：优惠金额中商家承担的部分		
				*/
               
                
				if($sendtype == 1){   
				//自送  （商品总价 + 打包费 + 配送费 - 优惠金额中商家承担的部分）* 佣金比例
				$value['yje'] = $value['yb']*($value['goodscost']  + $value['bagcost'] + $value['pscost'] - $value['shopcxcost']);	
				}else{                
				//网站配送 （商品总价 + 打包费 - 优惠金额中商家承担的部分）* 佣金比例
				$value['yje'] = $value['yb']*($value['goodscost']  + $value['bagcost'] - $value['shopcxcost']);	
				} 
    ...
    function outtjorder()
    {
        ...
        if(is_array($shoplist))
        {
            foreach($shoplist as $key=>$value)
            {
				...
				if($sendtype == 1){
					$value['yje'] = $value['yb']*($value['goodscost'] + $value['pscost'] + $value['pscost'] - $value['shopcxcost'] );
				}else{
					$value['yje'] = $value['yb']*($value['goodscost']+ $value['pscost'] - $value['shopcxcost'] );
				}
