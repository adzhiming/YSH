...
{  
   ...
   #      $where = empty($where)?'   and  SQRT((`lat` -'.$lat.') * (`lat` -'.$lat.' ) + (`lng` -'.$lng.' ) * (`lng` -'.$lng.' )) < (`pradius`*0.01094-0.01094) ': $where.' and SQRT((`lat` -'.$lat.') * (`lat` -'.$lat.' ) + (`lng` -'.$lng.' ) * (`lng` -'.$lng.' )) < (`pradius`*0.01094-0.01094) ';
				...
				# $temb[] = $this->mysql->getarr("select * from ".Mysite::$app->config['tablepre']."shopmarket as a left join ".Mysite::$app->config['tablepre']."shop as b  on a.shopid = b.id  where  b.is_pass = 1 and b.is_recom = 1 ".$where."    order by ".$orderarray[$order]." limit ".$pageinfo->startnum().", ".$pageinfo->getsize()."");
					...
					//and b.is_recom != 1
				 $temb[] = $this->mysql->getarr("select * from ".Mysite::$app->config['tablepre']."shopmarket as a left join ".Mysite::$app->config['tablepre']."shop as b  on a.shopid = b.id  where  b.is_pass = 1  ".$where."    order by ".$orderarray[$order]." limit ".$pageinfo->startnum().", ".$pageinfo->getsize()."");
  ...
  #     $where = empty($where)?'   and  SQRT((`lat` -'.$lat.') * (`lat` -'.$lat.' ) + (`lng` -'.$lng.' ) * (`lng` -'.$lng.' )) < (`pradius`*0.01094-0.01094) ': $where.' and SQRT((`lat` -'.$lat.') * (`lat` -'.$lat.' ) + (`lng` -'.$lng.' ) * (`lng` -'.$lng.' )) < (`pradius`*0.01094-0.01094) ';
	 ...
	 //市场商家列表
	 function marketshoplistdata(){
	     $typelx = IFilter::act(IReq::get('typelx'));
	     $marketid = IFilter::act(IReq::get('marketid'));
	     if(!empty($typelx)){
	         if($typelx == 'wm'){
	             ICookie::set('shopshowtype','waimai',2592000);
	             $shopshowtype = 'waimai';
	     ...
	     if($shopshowtype == 'market'){
	         ...
	         $shopsearch = IFilter::act(IReq::get('search_input'));
	         $shopsearch = urldecode($shopsearch);
	         if(!empty($shopsearch)) $where=" and b.name like '%".$shopsearch."%' ";
	         $areaid= intval(IFilter::act(IReq::get('areaid'))); //  ICookie::get('myaddress');
	         $catid = intval(IReq::get('catid'));
	         $order = intval(IReq::get('order'));
	         $order = in_array($order,array(1,2,3))? $order:0;
	...
	//'0'=>' (`lat` -'.$lat.') * (`lat` -'.$lat.' ) + (`lng` -'.$lng.' ) * (`lng` -'.$lng.' ) ASC       ',                       
		...
		$pageinfo->setpage(intval(IReq::get('page'))); 
 		$where = Mysite::$app->config['plateshopid'] > 0? $where.' and  id != '.Mysite::$app->config['plateshopid'] .' ':$where;
     	 $tempdd = array();
		 //and is_recom = 1
		$tempdd[] =   $this->mysql->getarr("select * from ".Mysite::$app->config['tablepre']."shop where is_pass = 1  and is_open = 1  and endtime > ".time()."  ".$where."    order by ".$orderarray[0]." limit ".$pageinfo->startnum().", ".$pageinfo->getsize()."  ");		
...
function makeorder(){
	...
	$checkps = 	 $this->pscost($shopinfo,$temp['lng'],$temp['lat']);
     ...
     if(isset($ipaddress["area1"])){
		   ...
		   $info['ipaddress']  = $ipaddress['ip'].iconv("GBK", "UTF-8", $ipaddress["area1"]);
	   } 
...
/* 搜索店铺 结果  */
   ...
   $where = empty($where)?'   and  SQRT((`lat` -'.$lat.') * (`lat` -'.$lat.' ) + (`lng` -'.$lng.' ) * (`lng` -'.$lng.' )) < (`pradiusa`*0.01094-0.01094) ': $where.' and SQRT((`lat` -'.$lat.') * (`lat` -'.$lat.' ) + (`lng` -'.$lng.' ) * (`lng` -'.$lng.' )) < (`pradiusa`*0.01094-0.01094) ';
					...
					$tempdd = array();
					//and is_recom = 1
					$tempdd[] =   $this->mysql->getarr("select * from ".Mysite::$app->config['tablepre']."shop where is_pass = 1  and (   admin_id='".$this->CITY_ID."'  or  admin_id = 0 )   and is_open = 1   and endtime > ".time()."  ".$where." ");		
...
/* 8.3新增  2016-05-31  zem */
	...
	function receivejuan(){    //下单成功分享优惠券 根据手机号  领取优惠券
		...
		if( !empty($sendjuanlist) ){
						foreach($sendjuanlist as $key=>$val){ 
							...
							$data['endtime'] = time()+($checkinfosendjuan['endtime']*24*60*60);
							if( !empty($memberinfo) ){
	...
	function memsharej(){   //会员忠心推广分享优惠券页面
		...
		$where = "  where `type` = 3 and addtime < '".time()."' ";
 		$checkinfosendjuan = $this->mysql->select_one("select * from ".Mysite::$app->config['tablepre']."juanrule ".$where." order by orderid asc ");
 ...
 function memsharehb(){   //会员忠心推广分享领取优惠券页面
		...
		$where = "  where type=3 and addtime < ".time()." and is_open = 1 and juannum = 1 ";
 		$checkinfosendjuan = $this->mysql->select_one("select * from ".Mysite::$app->config['tablepre']."juanrule ".$where." order by orderid asc ");
...
function gzwx(){    
	...
	function receivgzwxjuan(){    
		...
		if( !empty($checkisrec) )  {
	 	...
	 	$where = "  where type=4 and addtime < ".time()."  and is_open = 1 and juannum > 0 ";
 		$checkinfosendjuan = $this->mysql->select_one("select * from ".Mysite::$app->config['tablepre']."juanrule ".$where." order by orderid asc ");
 		$sendjuanlist = array();
		if(!empty($checkinfosendjuan)){	 	
				for($i=1;$i<=$checkinfosendjuan['juannum'];$i++ ){  
