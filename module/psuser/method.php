   ...
   //8.6修改
    function ordercontrol(){
  	 ...
  	 if($dotype == 'joinorder'){//加入订单
			...
			$psset = $this->ordmysql->select_one("select *  from ".Mysite::$app->config['tablepre']."platpsset  where cityid= '".$ordercheck['admin_id']."'   ");
			...
			if($ordercheck['buyeruid'] > 0){
				...
				$appcheck =  $this->mysql->select_one("select *  from ".Mysite::$app->config['tablepre']."appbuyerlogin where uid = '".$ordercheck['buyeruid']."'   ");  
				$tempuser[] = $appcheck;
				$appCls->SetUserlist($tempuser)->sendNewmsg('配送已取货','配送员开始配送');
