   ...
   function applogin(){
	...
	function waitorder(){
	   ...
	   foreach($psorderlist as $key=>$value){
			...
			//is_reback   提成费用
			//  
			$value['is_yuding'] = $orderinfo['posttime'] - $orderinfo['addtime'] > 1800?'1':'2';
			...
			$value['daycode'] =  $orderinfo['daycode']; //当天序号
		    $templist[] = $value;
...
//			$this->message('nologin');
//		}
		$where = " where ordps.status = 3 and ordps.psuid = 83";
		...
		$psorderinfo = $this->mysql->getarr("SELECT ordps.dno,ordps.addtime,ordps.pstime,ordps.picktime,ordps.psycost,ord.shopname,ord.shopphone,ord.paystatus,ord.shopps,ord.allcost,ord.shopaddredd,ord.buyeraddress,ord.buyerphone FROM ".Mysite::$app->config['tablepre']."orderps as ordps left join ".Mysite::$app->config['tablepre']."order as ord  on ordps.dno = ord.dno ".$where." ");
    ...
    function ordercontrol(){
  	 ...
  	 if($dotype == 'joinorder'){//加入订单
			...
			if($ordercheck['buyeruid'] > 0){
				...
				if(!empty($appcheck)){ 
					  $appCls->sendmsg($appcheck['userid'],$appcheck['channelid'],"配送员已抢单",$data['psusername'].'抢单成功,联系电话'.$backinfo['phone'],1);
				}
			}
  	 	    $this->success('抢单成功');
			...
			if($ordercheck['buyeruid'] > 0){
				...
				$appcheck =  $this->mysql->select_one("select *  from ".Mysite::$app->config['tablepre']."appbuyerlogin where uid = '".$ordercheck['buyeruid']."'   ");  
				if(!empty($appcheck)){ 
					  $appCls->sendmsg($appcheck['userid'],$appcheck['channelid'],"配送已取货",'配送员开始配送',1);
				}				
			 }
		    $this->success('取单成功'); 
       ...
       $juandata['name'] = '推荐送优惠券';//  优惠券名称
		    ...
		    if($checkinfo['buyeruid'] > 0){
				...
				if(!empty($appcheck)){ 
					  $appCls->sendmsg($appcheck['userid'],$appcheck['channelid'],"配送员已送达",'配送完成',1);
				}
		   }
  	 	   $this->success('操作成功');
  ...
  function start_work(){
	...
	function close_work(){
		...
		$userid = trim(IFilter::act(IReq::get('userid')));
		if(!empty($userid)){ 
				$this->mysql->delete(Mysite::$app->config['tablepre']."apploginps","userid='".$userid."'"); 
		} 
