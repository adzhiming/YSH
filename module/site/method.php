...
{  
	 function index(){
        ...
        $templist = $this->mysql->getarr("select * from ".Mysite::$app->config['tablepre']."shoptype where  cattype = 0 and parent_id = 0 and is_search =1  order by orderid asc limit 0,1000");
 			...
 			$data['goodstypeid']  =  $this->mysql->select_one("select * from ".Mysite::$app->config['tablepre']."shoptype where  id = ".$cate." ");
	 ...
	 $shopcounts = $this->mysql->select_one( "select count(id) as shuliang  from ".Mysite::$app->config['tablepre']."order	 where   status = 3 and  shopid = ".$value['id']."" );
		...
		#	print_r(  $data['shoplist']  );
			...
			/* 分类改变 */
			$moretypelist = $this->mysql->getarr("select* from ".Mysite::$app->config['tablepre']."appadv where type=2 and activity = 'waimai' and param != 'lifehelp'  and (   cityid='".$this->CITY_ID."'  or  cityid = 0 ) order by orderid  asc");
	 	    ...
	 	    //搜索信息
	 	     ...
	 	     if($locationtype == 1){
	 	        ...
	 	        $lat = empty($lat)?0:$lat;   
		          ...
		          $bili = $bili*0.009;
		    #      $where .= empty($where) ? ' where id > 0 and endtime > '.time().' and  SQRT((`lat` -'.$lat.') * (`lat` -'.$lat.' ) + (`lng` -'.$lng.' ) * (`lng` -'.$lng.' )) < (`pradius`*0.01094 -0.01094 ) ':' and id > 0 and endtime > '.time().'  and SQRT((`lat` -'.$lat.') * (`lat` -'.$lat.' ) + (`lng` -'.$lng.' ) * (`lng` -'.$lng.' )) < (`pradius`*0.01094 -0.01094 ) ';//0.009/6 = 0.0015
		     }else{
	 	    	//文字定位
	 	    	  $nowadID = ICookie::get('myaddress'); 
	         $myaddressname= ICookie::get('mapname');  
